...
def create_command_stack(image, receiver):
    ...
    data = ("Missing packet re-request command stack\n" +
            f"generated {str(current_time)}\n" +
            ...
            f"Total commands to issue: {len(missing_packets)}\n\n"
    ...
    for packet in missing_packets:
        ...
        if packet not in missing_packets_excluding_corrupted:
            reason = "corrupted"
        data = data + ("%i\t\t\t%s\t%i\t\t\t%i\t\t\t\t%s\n" %
                       (image.ec_address, ec_position + ' '*(8-len(ec_position)),
                        image.memory_slot, packet, reason))
    data = data + f'\nGenerated by WAPS IES {receiver.conf["version"]}'
